...
class AddRecipeContainer extends React.Component {
  ...
  messages = {
    ...
    photo:
      "Dodaj link do zdjęcia zawierajacy na początku protokół http:// lub https://",
  ...
  onInputChangeHandler(input) {
    return event => {
      const valueToSave =
        event.value || (event.target && event.target.value) || "";
      this.setState({
        [input]: valueToSave
      });
    };
    ...
    return (
      <Paper style={style.paper}>
        <div className="addRecipeContainer">
          <Typography variant="h3">Dodaj swój przepis !</Typography>
        ...
        <div>
          ...
          {errors.isNutritiveValue && (
            <span> {this.messages.nutritiveValue} </span>
          )}
        </div>
        <div>
